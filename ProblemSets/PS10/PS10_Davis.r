...
cv <-folds <- vfold_cv(income_train, v =3)

rec_wf <- workflow() %>%
  add_model(tune_tree_spec) %>%
  add_formula(high.earner ~ education + marital
...
top_acc  <- show_best(tune_res, metric = "accuracy")
...
# YOU FILL IN THE REST
knncv <- vfold_cv(income_train, v = 3)
...
svc_folds <- vfold_cv(income_train, v = 3)
